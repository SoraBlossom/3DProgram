...
void Application::Update()
{
	...
	{
		// 回転
		const float rotSpd = 1.5f;		// 回転速度
		if (GetAsyncKeyState(VK_UP) & 0x8000) m_cameraRotX -= rotSpd;
		if (GetAsyncKeyState(VK_DOWN) & 0x8000) m_cameraRotX += rotSpd;
		if (GetAsyncKeyState(VK_LEFT) & 0x8000) m_cameraRotY -= rotSpd;
		if (GetAsyncKeyState(VK_RIGHT) & 0x8000) m_cameraRotY += rotSpd;
		if (m_cameraRotX > 90.0f) m_cameraRotX = 90.0f;
		if (m_cameraRotX < -90.0f) m_cameraRotX = -90.0f;
		if (m_cameraRotY > 360.0f) m_cameraRotY -= 360.0f;
		if (m_cameraRotY < 0.0f) m_cameraRotY += 360.0f;

		// 移動
		const float movSpd = 0.3f;		// 移動速度
		...
		move *= movSpd;
		float moveY = 0.0f;
		if (GetAsyncKeyState(VK_SPACE) & 0x8000) moveY += movSpd;
		if (GetAsyncKeyState(VK_LSHIFT) & 0x8000) moveY -= movSpd;

		m_cameraMat = Math::Matrix::CreateRotationX(DirectX::XMConvertToRadians(m_cameraRotX)) *
			Math::Matrix::CreateTranslation(move) *
			Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(m_cameraRotY)) *
			Math::Matrix::CreateTranslation(0, moveY, 0) *
			Math::Matrix::CreateTranslation(m_cameraMat.Translation());

		m_spCamera->SetCameraMatrix(m_cameraMat);
	}
...
bool Application::Init(int w, int h)
{
	...
	m_cameraMat = Math::Matrix::CreateTranslation(0, 15, -20);
	m_cameraRotX = 40.0f;
	m_cameraRotY = 0;
